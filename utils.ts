 ...
 */
export function createHyperSubLayer(sublayer_key: KeyCode, commands: HyperKeySublayer, allSubLayerVariables: string[], to_if_alone?: To[], to_if_held_down?: To[]): Manipulator[] {
...
export function createHyperSubLayers(subLayers: subLayersType): KarabinerRules[] {
  const allSubLayerVariables = (Object.keys(subLayers) as (keyof typeof subLayers)[]).map((sublayer_key) => generateSubLayerVariableName(sublayer_key));
  ...
  return Object.entries(subLayers).map(([key, value]) => {
    if ("to" in value)
      ...
      return {
        ...
        manipulators: createHyperSubLayer(key as KeyCode, value as HyperKeySublayer, allSubLayerVariables, to_if_alone, to_if_held_down),
      };
    }
  });
}
...
export function L(key_code: KeyCode, modifiers: To["modifiers"], description?: string): LayerCommand {
